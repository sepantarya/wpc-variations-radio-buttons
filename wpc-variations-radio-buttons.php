...
! defined( 'WOOVR_VERSION' ) && define( 'WOOVR_VERSION', '3.3.2' );
	...
	function woovr_init() {
		...
		if ( ! class_exists( 'WPClever_Woovr' ) && class_exists( 'WC_Product' ) ) {
			class WPClever_Woovr {
				...
				function __construct() {
					...
					add_action( 'before_woocommerce_init', function () {
						if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
							FeaturesUtil::declare_compatibility( 'custom_order_tables', WOOVR_FILE );
						}
					} );
				}
							...
							<?php if ( $active_tab === 'settings' ) {
								...
								<form method="post" action="options.php">
									<table class="form-table">
										...
										<tr>
											...
											<td>
												...
												<div class="woovr_clear_image_custom" style="display: none">
													...
													<span class="woovr_clear_image_preview" id="woovr_clear_image_preview">
                                                        <?php if ( $clear_image_id ) {
	                                                        echo '<img src="' . wp_get_attachment_url( $clear_image_id ) . '"/>';
                                                        } ?>
                                                    </span>
													<input id="woovr_clear_image_upload" type="button" class="button" value="<?php esc_attr_e( 'Upload image', 'wpc-variations-radio-buttons' ); ?>"/>
													<input type="hidden" name="woovr_settings[clear_image_id]" id="woovr_clear_image_id" value="<?php echo esc_attr( $clear_image_id ); ?>"/>
												</div>
											</td>
										</tr>
										<tr>
										...
										<tr>
											<td colspan="2">
												If you want to insert the message to some places on Variations Radio Buttons, please try another free plugin from us
												<a href="https://wordpress.org/plugins/wpc-smart-messages/" target="_blank">WPC Smart Messages for WooCommerce</a>.
											</td>
										</tr>
										<tr class="submit">
											<th colspan="2">
				...
				public static function woovr_variations_form( $product, $variation = false, $context = '' ) {
					$product_id         = $product->get_id();
							...
							if ( $selector === 'default' || $selector === 'grid' || $selector === 'grid-2' || $selector === 'grid-3' || $selector === 'grid-4' ) {
								...
								foreach ( $children_data as $child_data ) {
									...
									if ( $child_product->get_image_id() ) {
										...
										$child_image_src = get_post_meta( $child_id, 'woovr_image', true ) ?: $child_image[0];
